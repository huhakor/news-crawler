 ...
 *
 * update: 20250429
 * - const SELECTORS를 통해 구조 개편 위험성이 큰 태그들을 모아 html 구조 변경에 안정적으로 대비
 * - '관련 뉴스 n개 전체보기' 링크 있는 뉴스 그룹 처리 추가와 그에 따른 전체 수집 프로세스 개편(하위 기사 목록 수가 많으면 전용 링크에 접속해야만 전체 관련 뉴스 수집이 가능해짐)
 * - 상위, 하위 뉴스 html 구조 변경에 따른 수정
 * - 네이버 뉴스 날짜 수정(created&updatedDate)
 * - p-limit을 통한 제한적 병렬작업(과부하 방지 및 속도 개선)
 * update: 20250506
 * - 상위 및 하위 뉴스 분리 추출 정교화
 * - 날짜 selector 업데이트 및 형식 변환(통일) 적용 (한 달 이내 최근 뉴스의 경우 'n주 전', 'n일 전', 'n시간 전'으로 표시됨 -> 계산하여 datetime으로 변환)
 * - 한 요소의 여러 selector 중 하나만 반영하는 로직 적용
 * update: 20250510
 * - selector 업데이트
 * - 중복 뉴스 추출 방지 도와주는 utility function 추가(기존 seenUrl 처리 + *관련뉴스n개 전체보기에서 처리했을 시 다시 뉴스 그룹에서 처리 x 유지 + 수집한 url 중 동일한 것 다시 수집 x)
 */
// ===============================
// 모듈 및 라이브러리 임포트
// =============================== 
 ...
 */
const SELECTORS = {
  newsGroup: 'div.sds-comps-vertical-layout.sds-comps-full-layout.dZQQMujvOqnxG1bUQsg6',
  relatedLink: 'a[href*="related=1"]',
  subNewsOuter: 'div.sds-comps-base-layout.sds-comps-full-layout.m7VJzZ2bSaunam2mv1Xd',
  subNewsItem: 'div.sds-comps-base-layout.sds-comps-full-layout.AgmDmsKtWKXsclHTvTwY', //':scope > div.sds-comps-base-layout.sds-comps-full-layout'
  top: {
    profile: 'div[data-sds-comp="Profile"]', //업데이트 profile&content 추가
    content: 'div.sds-comps-base-layout.sds-comps-full-layout.m6eR7KmFjLYOLhYRyXPh',
    ...
    title: 'a[class*="cdv6mdm2"] > span.sds-comps-text-ellipsis-1',
    summary: 'span.sds-comps-text-ellipsis-3.sds-comps-text-type-body1',
    pressUrl: 'a[href^="http"]:not([href*="n.news.naver.com"])',
    naverUrl: '.sds-comps-profile-source a[href*="n.news.naver.com"]' //'a[href*="n.news.naver.com"]'
  },
  sub: {
    press: ':scope .sds-comps-profile-info-title > span.sds-comps-text-type-body2',
    date: ['span.sds-comps-profile-info-subtext > span.sds-comps-text-type-body2',
            'span.sds-comps-text.sds-comps-text-type-body2.sds-comps-text-weight-sm'].join(', '),
    title: ':scope a span[class*="ellipsis"]',
    pressUrl: ':scope a[href^="http"]:not([href*="naver"])',
    naverUrl: ':scope a[href*="n.news.naver.com"]'
  },
  article: {
